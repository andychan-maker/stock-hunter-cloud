            ...
            try:
                ...
                if not df.empty:
                    ...
                    if not df.empty:
                        ...
                        # --- 4. SMART UPLOAD (No Duplicates) ---
                        sheet = get_google_sheet()
                        
                        # Step A: Get all existing data
                        existing_data = sheet.get_all_records()
                        existing_df = pd.DataFrame(existing_data)
                        
                        # Step B: Create a "Checklist" of what's already there
                        # We combine Date + Ticker into a unique key (e.g. "2023-10-25-AAPL")
                        existing_keys = set()
                        ...
                        # Step C: Filter the NEW data
                        data_to_upload = []
                        duplicates_count = 0
                        
                        for index, row in final_df.iterrows():
                            # check if this specific Date+Ticker exists
                            check_key = (str(row['Date']), str(row['Ticker']))
                        ...
                        # Step D: Upload only if we have new data
                        if data_to_upload:
                            sheet.append_rows(data_to_upload)
                            st.success(f"✅ Success! Added {len(data_to_upload)} NEW stocks.")
                            if duplicates_count > 0:
                                st.info(f"(Skipped {duplicates_count} stocks that were already in the sheet for today)")
                        else:
                            st.warning("⚠️ No new stocks added. (All found stocks are already in the sheet!)")
            ...
            if not history_df.empty:
                ...
                with col1:
                    ...
                    if not week_df.empty:
                        counts = week_df['Ticker'].value_counts()
                ...
                with col2:
                    ...
                    if not month_df.empty:
                        counts = month_df['Ticker'].value_counts()
                        st.dataframe(counts, use_container_width=True)
